...
function gateKeeper(req, res, next) {
  ...
  const headerVal = req.get('x-username-and-password');
  const parseHeader = queryString.parse(headerVal);
  parseHeader.toLowerCase;
  console.log(parseHeader.pass)
  console.log(parseHeader.user)
  for(let i=0; i<USERS.length; i++) {
  if(USERS[i].password === parseHeader.pass && USERS[i].userName === parseHeader.user) {
    req.user = USERS[i];
    }
